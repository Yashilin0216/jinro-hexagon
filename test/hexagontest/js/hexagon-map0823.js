...
createApp({
  setup() {
    ...
    const pos_px = computed(() => {
      const p = axial_to_pixel(player.q, player.r, cfg.hex_size);
    ...
    // 指定セルが現在選択されているか判定
    function is_selected(cell) { return cell.q === player.q && cell.r === player.r; }

    // 指定セルがホバーされているか判定
    function is_hover(cell) { return hover.value && hover.value.q === cell.q && hover.value.r === cell.r; }

    // 「半径Rマス以内」を判定する処理 a,bはそれぞれが座標 リターンは距離 move_restrictions.jsにも同様の関数あり
    ...
    function on_click(evt) {
      ...
      /* && three_restriction.canMove(player, { q: ar.q, r: ar.r },player.name)*/
    ...
    return { cfg, grid, hover, camera, view_box, pos_px, hex_points, tile_fill, is_selected, on_key, on_mouse_move, on_click, board_ref,axial_to_pixel,
            // ▼▼▼ 追加 ▼▼▼
